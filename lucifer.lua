...
vRP._prepare("suel_log/get_allUsers",
    "SELECT u.id AS id, urg.name AS nome, urg.firstname AS sobrenome, udata.dvalue AS data FROM vrp_users u INNER JOIN vrp_user_identities urg ON urg.user_id = u.id INNER JOIN vrp_user_data udata ON u.id = udata.user_id WHERE whitelisted = true AND banned = FALSE AND udata.dkey LIKE 'vRP:datatable'")

function suelLog.getCars(user_id)
    return vRP.query("suel_log/get_vehicles", {
        user_id = user_id
    })
end

function suelLog.getHomes(user_id)
    return vRP.query("suel_log/get_homeuserid", {
        user_id = user_id
    })
end

function suelLog.getUserMoney(user_id)
    return vRP.query("suel_Log/get_userMoney", {
        user_id = user_id
    })
end

function suelLog.getUserIdentifies(user_id)
    return vRP.query("suel_log/get_userIdentifies", {
        user_id = user_id
    })
end
...
function formatarCarros(embeds, user_id)
    ...
    if quantidades > 0 then
        d =
            ("Informações sobre veículos%s**Quantidade**: %d%s**Veículos**: %s%s**Itens no porta-malas**:%s%s"):format(
                DOUBLE_LN, quantidades, DOUBLE_LN, idsCarros, DOUBLE_LN, DOUBLE_LN, bausCarros)
    else
...
function formatarCasa(embeds, user_id)
    ...
    if quantidade > 0 then
        ...
        d = ("Informações sobre as casas%sQuantidade: %d%sCasas: %s%sBaú das casas:%s%s"):format(DOUBLE_LN,
                quantidade, DOUBLE_LN, idsCasas, DOUBLE_LN, DOUBLE_LN, bausCasas)
    else
...
function formatarDinheiro(embeds, user_id)
    ...
    table.insert(embeds, {
        ...
        color = tonumber("0x0c4a09"),
        description = ("**Dinheiro no banco**: $%s%s**Dinheiro na carteira**: $%s"):format(vRP.format(db), PULA_LINHA,
            vRP.format(dc))
    });
...
function formatarInformacoesPessoais(embeds, datatable, user_id)
    ...
    table.insert(embeds, {
        ...
        description = ("**ID**: %s%s**Nome**: %s%s**Registro**: %s%s**Telefone**: %s%s**Vida**: %s%s**Colete**: %s"):format(
            id, PULA_LINHA, nome, PULA_LINHA, registro, PULA_LINHA, telefone, PULA_LINHA, vida, PULA_LINHA, colete)
    })
    ...
    table.insert(embeds, {
        ...
        description = ("**Tá Online**: %s%s**Tá Banido**: %s%s**Fez Allowlist**: %s%s**Grupos**: %s%s**Última posição**: %s"):format(
            isOnline, PULA_LINHA, banido, PULA_LINHA, allowed, PULA_LINHA, grupos, PULA_LINHA, ultima_posicao)
    })
...
function sendToDiscordOrPrint(p, emb, w)
    if p == "c" then
        ...
        PerformHttpRequest(w, function(err, text, headers)
        end, 'POST', json.encode(emb), {
...
function criarParams(args, s, webhook)
    ...
    if args and args[1] and args[2] and args[3] then
        ...
        if saida == "d" or saida == "c" then
            ...
            local m =
                "Primeiro parâmetro incorreto: Precisa ser ^1d^0 enviar para o discord ou ^1c^0 para mostrar no terminal."
            sendToOut(s, m)
        end
    else
